...
def generate_cert_chain(directory: str, length: int = 1):
    ...
    outputdata = subprocess.run(["ls","-l",directory], capture_output=True, text=True, check=True)
    print(outputdata)
    print(r.stdout)
    print(r)
    print(r.returncode)
    ...
    if r.returncode != 0:
        ...
        print('exiting without creating certificate')
        sys.exit(1)
...
class TestCase(abc.ABC):
    ...
    def certs_dir(self):
        ...
        print("inside cert_dir" + self._cert_dir.name + "/")
    ...
    def _keylog_file(self) -> str:
        ...
        print('inside keylogFile:server' + self._server_keylog_file + 'client' + self._client_keylog_file)
    ...
    def _check_version_and_files(self) -> bool:
        print('inside check version function')
        versions = [hex(int(v, 0)) for v in self._get_versions()]
        print("get versions cleared")
        print(versions)
        if len(versions) != 1:
    ...
    def _get_versions(self) -> set:
        ...
        tr = self._server_trace()
        print('inisde get versions function')
...
class TestCaseHandshake(TestCase):
    ...
    def check(self) -> TestResult:
        print('inisde Handshake test check')
        if not self._check_version_and_files():
            print('after check version')
            return TestResult.FAILED
        print('before retry sent')
        if self._retry_sent():
            logging.info("Didn't expect a Retry to be sent.")
            return TestResult.FAILED
        
        print('before handshakes')
